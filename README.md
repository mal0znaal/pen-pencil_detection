...
Цель: обучить и сравнить несколько моделей YOLO для детекции двух классов — **ручка** (`pen`) и **карандаш** (`pencil`) — на своих данных, а затем проверить качество на отдельном тестовом видео (или в реальном времени).
...
Результаты на валидационной выборке оказались хорошими, однако для точной оценки результата стоит проверить как модель покажет себя в боевых условиях (второе видео, записанное для тестирования).  
...
И вау, модель распознала карандаш как `карандаш`, но ошиблась с ручками)).
...
1. Добавим в обучающую выборку еще **10** изображений `проблемного` карандаша с различных ракурсов. Новый датасет: https://app.roboflow.com/testproject-64hot/pen_penicl1-4izcj/3  
  P.S. Фотографии были сделаны дополнительно, никаких data leaks. 
...
#### Подробно сравним три лучшие модели:

| Model     | Layers | Params (M) | GFLOPs | Precision (P) | Recall (R) |   F1   | mAP@0.5 | mAP@0.5:0.95 |
|-----------|--------|-----------:|-------:|---------------|-----------:|-------:|--------:|-------------:|
| YOLO12s   | 159    |      9.23  |  21.2  | 0.966         | 0.972      | 0.9691 | 0.980   | 0.667        |
| YOLO11s   | 100    |      9.41  |  21.3  | 0.916         | 0.959      | 0.9370 | 0.978   | 0.690        |
| RT-DETR-L | 302    |     31.99  | 103.4  | 0.966         | 0.959      | 0.9629 | 0.960   | 0.579        |



#### Сравнение моделей по классам

| Model     | Class  | P      | R      |   F1    | mAP@0.5 | mAP@0.5:0.95 |
|-----------|--------|--------|--------|--------:|--------:|-------------:|
| YOLO12s   | all    | 0.966  | 0.972  | 0.9691  | 0.980   | 0.667        |
| YOLO12s   | pen    | 0.993  | 1.000  | ------- | 0.995   | 0.714        |
| YOLO12s   | pencil | 0.938  | 0.944  | ------- | 0.965   | 0.621        |

| Model     | Class  | P      | R      |   F1    | mAP@0.5 | mAP@0.5:0.95 |
|-----------|--------|--------|--------|--------:|--------:|-------------:|
| RT-DETR-L | all    | 0.966  | 0.959  | 0.9629  | 0.960   | 0.579        |
| RT-DETR-L | pen    | 0.933  | 0.923  | ------  | 0.926   | 0.695        |
| RT-DETR-L | pencil | 1.000  | 0.995  | ------  | 0.995   | 0.463        |

| Model     | Class  | P      | R      |   F1    | mAP@0.5 | mAP@0.5:0.95 |
|-----------|--------|--------|--------|--------:|--------:|-------------:|
| YOLO11s   | all    | 0.916  | 0.959  | 0.9371  | 0.978   | 0.690        |
| YOLO11s   | pen    | 0.935  | 0.962  | ------  | 0.989   | 0.688        |
| YOLO11s   | pencil | 0.896  | 0.957  | ------  | 0.968   | 0.692        |



В итоге, **YOLO12s** очевидно лучшая модель.  
**RT-DETR** мог бы быть хорош, но не на этих данных - тут его возможности не раскрылись.  

Из интересного: **RT-DETR-L** безошибочно определяет `карандаши` (не делает ложных предсказаний), а **YOLO12s** безошибочно определяет все `ручки` (не пропускает ни одну ручку).  Шутки ради можно было бы попробовать сделать **ансамбль** этих двух моделей.
...
## 5. Окружение и железо

Обучение и эксперименты проводились в следующем окружении:
...
- Обучение/инференс всех моделей выполнялись **на GPU**
